...
log_message('trail - sitecustomize (%s)', __file__)
...
# Python installation as the 'trail' script was installed in. This
# is necessary because if it isn't and we were now running out of a
# different Python installation, then it may not have the 'trail'
...
expected_python_prefix = os.environ.get('TRAIL_PYTHON_PREFIX')
actual_python_prefix = os.path.realpath(os.path.normpath(sys.prefix))

expected_python_version = os.environ.get('TRAIL_PYTHON_VERSION')
...
if python_prefix_matches and python_version_matches:
    # When the 'trail' script is run from out of a Python egg
    ...
    # 'trail' package, we need to ensure that in this case the
    # egg directory for 'trail' is manually added to 'sys.path'
    ...
    from trail.bootstrap import bootstrap
    from trail.bootstrap import register_bootstrap_functions

    # If we had to add the egg directory above corresponding to the
    # 'trail' package, now remove it to ensure the presence of
